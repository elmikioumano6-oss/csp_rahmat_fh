...
from database.models import Base, User, AnneeScolaire, Classe, Eleve, EcheancePaiement
...
try:
    ...
    if not admin_check:
        sel = bcrypt.gensalt()
        hashed_pwd = bcrypt.hashpw("password123".encode('utf-8'), sel).decode('utf-8')
    ...
    if role == 'admin':
        if page == "Classes & Tarifs": afficher_classes()
            ...
            if page == "Année scolaire":
                st.subheader("📅 Gestion et Configuration des Années Scolaires")
                st.markdown("Définissez l'année académique active et suivez l'historique des sessions de l'établissement.")
                st.markdown("---")
                ...
                with col_form:
                    ...
                    with st.form("form_annee"):
                        nouvelle = st.text_input("Libellé de l'année", placeholder="ex: 2026-2027")
                        submit_annee = st.form_submit_button("Enregistrer et Activer", use_container_width=True)
                        
                        if submit_annee:
                            if nouvelle.strip():
                                db.query(AnneeScolaire).update({AnneeScolaire.active: False})
                                existante = db.query(AnneeScolaire).filter(AnneeScolaire.libelle == nouvelle.strip()).first()
                                if existante:
                                    existante.active = True
                                else:
                                    db.add(AnneeScolaire(libelle=nouvelle.strip(), active=True))
                                
                                db.commit()
                                st.toast("Année scolaire enregistrée et activée avec succès !", icon="✅")
                                st.rerun()
                            else:
                                st.warning("Veuillez saisir un libellé valide.")
                ...
                with col_list:
                    ...
                    if annees_list:
                        data_annees = [
                            {
                                "Libellé": a.libelle, 
                                "Statut": "🟢 Active" if a.active else "⚪ Inactive"
                            } 
                            for a in annees_list
                        ]
                        st.dataframe(pd.DataFrame(data_annees), use_container_width=True, hide_index=True)
                    else:
                        st.info("Aucune année scolaire enregistrée pour le moment.")
