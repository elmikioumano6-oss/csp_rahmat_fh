...
# ==========================================
# PERSONNALISATION AVANCÉE (VISIBILITÉ MAXIMALE)
# ==========================================
st.markdown("""
    <style>
    .stApp {
...
# ==========================================
# MIGRATIONS AUTOMATIQUES (EXÉCUTÉES EN PREMIER)
# ==========================================
with engine.connect() as connection:
    ...
    try:
        ...
        cols = [row[1] for row in res]
        if "cycle" not in cols: connection.execute(text("ALTER TABLE classes ADD COLUMN cycle TEXT;"))
        if "tarif_scolarite" not in cols: connection.execute(text("ALTER TABLE classes ADD COLUMN tarif_scolarite FLOAT DEFAULT 0.0;"))
        connection.commit()
    except Exception: pass
...
def verifier_et_creer_comptes_par_defaut():
    ...
    try:
        ...
        for data in comptes:
            ...
            if not user:
                user = User(username=data["username"], password=data["password"], role=data["role"], entite_id=data["entite_id"])
                db.add(user)
            else:
